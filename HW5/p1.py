...
class Island(object):
    ...
    def __init__(self, n, prey_count=0, predator_count=0, count_humans=0):
        ...
        self.init_animals(prey_count, predator_count, count_humans)

    def init_animals(self, prey_count, predator_count, count_humans):
        ...
        count = 0
        # same while loop but for count_humans
        while count < count_humans:
            x = random.randint(0, self.grid_size - 1)
            y = random.randint(0, self.grid_size - 1)
            if not self.animal(x, y):
...
###########################################
def main(human_breed_time=6, human_starve_time=3, human_hunt_time=4, initial_humans=10, predator_breed_time=6,
    ...
    for i in range(ticks):
        ...
        count_human = isle.count_humans()
        if prey_count == 0:
            print('Lost the Prey population. Quiting.')
            break
        prey_list.append(prey_count)
        predator_list.append(predator_count)
        human_list.append(count_human)
    print('*' * 20)
    print(isle)
    prey_length = len(prey_list)
    pylab.plot(np.array(range(0, prey_length)), np.array(predator_list[0:prey_length]), label="Predators")
    pylab.plot(np.array(range(0, prey_length)), np.array(prey_list[0:prey_length]), label="Prey")
    pylab.plot(np.array(range(0, prey_length)), np.array(human_list[0:prey_length]), label="Humans")
